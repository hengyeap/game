<template>
  <div id='container'>
    <van-nav-bar :title="pageTitle" left-text="" right-text="分享" left-arrow @click-left="back" @click-right="shareShow = true" />
    <div class="wrap">
      <span class="rule" @click="rulesShow = true"><img :src="ruleBtn" alt=""></span>
...
<!--            <img class="day8" v-if="item.day===8&&!isSign(item.date)" :src="day8" alt="">-->
            ...
            <img v-if="isSign(item.date)" :src="successImg" alt="">
...
<!--            <span class="buqian1" v-if="item.day===8&&!isSign(item.date)"></span>-->
      ...
      <div class="words">补签当天下单且单笔订单实付金额大于等于100元<br>即可获得一次补签机会</div>
      <div class="btns">
        <div class="search"><img @click="queryConditions" :src="searchBtn" alt=""><span>已补签{{initInfo.reissueQty}}天</span></div>
        <div class="signIn"><img @click="signInNow" :src="signInNowBtn" alt="">已连续签到{{initInfo.days}}天</div>
      </div>
      <div class="coupons">
        <img :src="coupons" alt="">
        <div class="couponsBox">
          <div v-for="(item,index) in checkData" class="couponsDays">
            <div class="dayImg">
              ...
              <img v-else :src="signSuccess" alt="">
            </div>
            <div style="font-size: 12px;text-align: center;color:#fed9b9;margin-bottom:0.2rem;">30元券</div>
            <div class="receiveImg">
              <img @click="toReceive(index,item.couponId)" v-if="!item.checked&&item.current" :src="receive" alt=""> <!--去领取-->
              <img v-if="item.checked&&item.current" :src="received" alt=""><!--已领取-->
              <img v-if="!item.current" :src="noReceive" alt=""><!--不能领取-->
            </div>
          </div>
...
<!--      </div>-->
    ...
    <div v-show="buqianShow" class="buqianConfirm">
      <div class="buqianContent">
        <b>本月已补签{{initInfo.reissueQty}}次<br>剩余{{(7 - initInfo.reissueQty)}}次机会</b>
        <span>你确认补签{{buqianDate.split("-")[1]}}号吗?</span>
        <div class="buqianBtns">
          <img @click="buqianShow = false" class="cancelBtn" :src="cancelBtn" alt="">
          <img @click="comfirmBuQian" class="confirmBtn" :src="confirmBtn" alt="">
        </div>
      </div>
    </div>

<!--    <div v-if="conditionShow" class="queryConditions">-->
<!--      <img class="queryConditionsImg" src="https://resource.yuedaoec.com/image/signInMarch/zigeBg.png" alt="">-->
<!--      <div class="conditionContent">-->
<!--        <div class="noZiGe" v-if="conditionList && conditionList.length == 0">你还没有补签机会！请先去下单哦~</div>-->
<!--        <div  v-for="(item, index) in conditionList" :key="index" :class="item.buQianZigeStatus?'ziGe':'ziGe1'">-->
<!--          {{index+1}}、{{item.date}}<br/> 订单编号:{{item.orderCode}} 获得{{item.qty}}次补签机会-->
<!--          <span v-if="!item.buQianZigeStatus">(已过期)</span>-->
<!--        </div>-->
<!--      </div>-->
<!--      <span class="closeCondition1" @click="conditionShow = false"></span>-->
<!--    </div>-->
    <div v-if="conditionShow" class="queryConditions">
      <div class="conditionContent">
        <b v-if="conditionList && conditionList.length == 0">你还没有补签机会！请先去下单哦~</b>
        <ul>
          <li :class="item.buQianZigeStatus?'ziGe':'ziGe1'" v-for="(item, index) in conditionList" :key="index">{{index+1}}、{{item.date}}<br/> 订单编号:{{item.orderCode}} 获得{{item.qty}}次补签机会<span v-if="!item.buQianZigeStatus">(已过期)</span></li>
        </ul>
      </div>
      <span class="closeCondition1" @click="conditionShow = false"></span>
    </div>
    ...
    export default create({
        ...
        data() {
            return {
                ...
                searchBtn: "https://resource.yuedaoec.com/image/signInMarch/searchBtn.png",
                ...
                day8: "https://resource.yuedaoec.com/image/signInMarch/day8.png?11",
                day8Img: "https://resource.yuedaoec.com/image/signInMarch/day8Img.png?11",
                ...
                checkData: [
                    {checked: false, current: false,daysNume:'7',couponId:'1291'},
                    {checked: false, current: false,daysNume:'14',couponId:'1292'},
                    {checked: false, current: false,daysNume:'21',couponId:'1293'},
                    {checked: false, current: false,daysNume:'28',couponId:'1294'},
                ],
                coupons38Status:false,
                clickable:true,
                // buQianZigeStatus:true
            };
        },
        created() {
        ...
        methods: {
            ...
            toReceive(index,couponId){
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/sendCoupon/userId.distribute?userId=" + this.uid).then(res => {
            ...
            toFindCouponByUserId(){
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/findCouponByUserId/userId.distribute?userId=" + this.uid).then(res => {
                    axios.get("https://appdistapi.yuedaoec.com/sendCoupon/activity/marchSign/findCouponByUserId/userId.distribute?userId=" + this.uid).then(res => {
                    if (res.data.code == "10000") {
                        this.checkData[0].checked=res.data.res.flag07
                        this.checkData[1].checked=res.data.res.flag14
                        this.checkData[2].checked=res.data.res.flag21
                        this.checkData[3].checked=res.data.res.flag28
                        this.coupons38Status=res.data.res.flag08
            ...
            init() {
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/getSignIn/userId.distribute?userId=" + this.uid).then(res => {
                    axios.get("https://appdistapi.yuedaoec.com/sendCoupon/activity/marchSign/getSignIn/userId.distribute?userId=" + this.uid).then(res => {
            ...
            toCheck38(){
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/sendCoupon08/userId.distribute?userId=" + this.uid).then(res => {
                axios.get("https://appdistapi.yuedaoec.com/sendCoupon/activity/marchSign/sendCoupon08/userId.distribute?userId=" + this.uid + '&date=0' + this.buqianDate).then(res => {
                    if (res.data.code == "10000") {
                        this.coupons38Status = true;
                        Toast("领取成功");
                    } else {
                        Toast(res.data.msg);
                    }
            ...
            isReissueSign(date) {
                for (let i = 0; i < this.initInfo.reissueDate.length; i++) {
                    if (this.initInfo.reissueDate[i] == date) {
                        return true;
                    }
                }
            },
            ...
            baokuanPros() {
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/queryPresentInfo.distribute").then(res => {
            ...
            getConditions() {
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/getReissuInfo/userId.distribute?userId=" + this.uid).then(res => {
                    axios.get("https://appdistapi.yuedaoec.com/sendCoupon/activity/marchSign/getReissuInfo/userId.distribute?userId=" + this.uid).then(res => {
                    console.log(111222,res.data.res)
                    this.conditionList = res.data.res.qualificationVos;
                        let date = new Date()
                        this.nowDate = date.Format('yyyy-MM-dd')
                        // console.log( this.nowDate,this.conditionList[0].date.substring(0, 10))
                        for(let i = 0;i<this.conditionList.length;i++){
                            this.conditionList[i].buQianZigeStatus=true
                            if(this.conditionList[i].date.substring(0, 10)!=this.nowDate){
                                this.conditionList[i].buQianZigeStatus=false
                                console.log(this.conditionList[i].date.substring(0, 10),this.nowDate,this.buQianZigeStatus)
            ...
            signInNow() {
                if (this.initInfo.nowDate.split("-")[0] != "03") {
                    ...
                    if (this.initInfo.signToday == true) {
                        ...
                        // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/signIn/userId?userId=" + this.uid).then(res => {
                            axios.get("https://appdistapi.yuedaoec.com/sendCoupon/activity/marchSign/signIn/userId?userId=" + this.uid).then(res => {
                            if (res.data.code == "10000") {
                                Toast("签到成功");
                                this.init();
                            }
            ...
            toBuQian(date) {
                console.log(this.initInfo.nowDate.split("-")[0]);
                if (this.initInfo.nowDate.split("-")[0] != "03") {
                    Toast("活动已结束!");
                } else {
                    if (this.initInfo.qualificationQty - this.initInfo.reissueQty > 0) {
                        this.buqianShow = true;
                        this.buqianDate = date;
                    } else {
                        if (this.initInfo.reissueQty < 7) {
                            Toast("你还没有补签机会！请先去下单哦~");
                        } else {
                            Toast("本月7次补签机会已用完！");
                        }
                    }
            ...
            comfirmBuQian() {
                // axios.get("http://localhost:8988/sendCoupon/activity/marchSign/reissueSign/userId.distribute?userId=" + this.uid + '&date=0' + this.buqianDate).then(res => {
                    axios.get("https://appdistapi.yuedaoec.com/sendCoupon/activity/marchSign/reissueSign/userId.distribute?userId=" + this.uid + '&date=0' + this.buqianDate).then(res => {
                    if (res.data.code == "10000") {
                        this.buqianShow = false;
                        Toast("补签成功");
                        this.init();
                    } else {
                        this.buqianShow = false;
...
<style>
  * {
  ...
  .wrap {
    background: url('https://resource.yuedaoec.com/image/signInMarch/bg.png') no-repeat center top;
    background-size: 100%;
    padding-top: 460px;
  }
