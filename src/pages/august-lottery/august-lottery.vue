  ...
  <div class="page">
    <div class="nav">
      <div @click="navBarClickLeft"
           class="back-box">
        <img src="https://public.tingmimi.net/image/timeGame/back-icon.png" alt="" class="back-icon">

      </div>

    </div>
    <div v-if="show">
      <img v-if="amountGreaterThan588 == false" class="bg"
           :src="'https://resource.yuedaoec.com/assets/august-lottery/{}/diamond-bg.png'.replace('{}',platform)"
           alt="">
      <img v-else-if="amountGreaterThan588 == true" class="bg"
           :src="'https://resource.yuedaoec.com/assets/august-lottery/{}/over588-diamond-bg.png'.replace('{}',platform)"
           alt="">
      <div class="content">
        <div class="box">

          <August-lottery-com class="lottery-com"
                              :platform=platform
          />
        </div>
...
<script>
    ...
    import AugustLotteryCom from './component/AugustLotteryCom';
    ...
    export default create({
        ...
        created() {
            ...
            if (this.isBlank(orderCode)) { // 数据异常[订单号不存在]
                return Dialog.alert({
                    title: this.texts[this.platform].dataException,
                    message: this.texts[this.platform].orderCodeNotFound
                }).then(() => {
                    hybrid.nav.back();
                });
            }
                ...
                this.reqwest(params)
                    .then(response => {
                        if (response.success) {
                            ...
                            const res = response.result.res;
                            if (res.amount >= res.startLine) {
                                this.amountGreaterThan588 = true;
                            } else {
                                this.amountGreaterThan588 = false;
                            }
                            this.show = true;
